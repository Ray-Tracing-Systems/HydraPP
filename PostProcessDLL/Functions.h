...
[[nodiscard]] float Distance(const float a, const float b);
[[nodiscard]] float Distance(const float x1, const float y1, const float x2, const float y2);
[[nodiscard]] float Max(const float value1, const float value2, const float value3);
[[nodiscard]] float Min(const float value1, const float value2, const float value3);
float3 Mediana(const float4* inData, const int size, const int a_width, const int a_height, const int x, const int y);
[[nodiscard]] int FloatToInt(const float inData);
...
void ChrommAberr(const float3& a_inData, float* a_outData1, float* a_outData2, const int a_width, const int a_height, const float a_chromAberr, const int x, const int y);
...
[[nodiscard]] std::vector<float> CreateGaussKernelWeights1D_HDRImage2(const int blurRadius);
...
[[nodiscard]] float WeightWhiteBalance(const float a_lum);
...
void DiffractionStars(const float3& a_data, float3& a_diffrStars,
 const float a_sizeStar,

 const int a_numRay,


 const int a_RotateRay,
